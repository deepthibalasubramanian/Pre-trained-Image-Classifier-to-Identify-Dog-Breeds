...
def print_results(results_dic, results_stats_dic, model, 
    ...
    print("\n** Percentage Results:")
    for filename in results_stats_dic:
        if filename.startswith("pct"):
            print(f"{filename}: {results_stats_dic[filename]:.1f}%")
    if print_incorrect_dogs:
        print("\n** Misclassified Dogs:")
        for filename in results_dic:
            if sum(results_dic[filename][3:]) == 1:
                print(f"Real: {results_dic[filename][0]:<30} Classifier: {results_dic[filename][1]:<30}")
